...
import SwiftDate
/// 企业新老版航班查询结果
struct CoFlightSearchResult:ALSwiftyJSONAble {
    /// 起飞城市名
    ...
    init(jsonData result:JSON){
        let firstFlightLeg =  result["airfares"].arrayValue.first?["flightInfos"].arrayValue
        self.takeOffCity = firstFlightLeg?.first?["takeOffCity"].stringValue ?? ""
        self.arriveCity = firstFlightLeg?.last?["arriveCity"].stringValue ?? ""
        self.departTakeoffDate = result["takeOffDate"].dateFormat(.unix)
        let flightList = result["airfares"].arrayValue.map{ FlightItem(jsonData: $0) }
        //同时修改默认排序 将符合条件的放在上方
        self.flightList = flightList.sorted{ f1,f2 in
            //如果都是符合 则比较时间 从早到晚排序
            if !f1.contraryPolicy && !f2.contraryPolicy{
                return f1.takeOffDateTime < f2.takeOffDateTime
            }else if f1.contraryPolicy && f2.contraryPolicy {
                return f1.takeOffDateTime < f2.takeOffDateTime
            }
            return f2.contraryPolicy
    ...
    struct FlightItem:ALSwiftyJSONAble {
        ...
        /// 计算是否为协议航班
        var isProtocol:Bool {
            return self.cabinList.contains{ $0.isProtocol}
        }
        ...
        init(jsonData:JSON){
            ...
            // =====计算是否跨天======
            let takeOffDate = jsonData["flightInfos"].arrayValue.first!["takeOffDate"].dateFormat(.unix)
            let arriveDate = jsonData["flightInfos"].arrayValue.last!["arriveDate"].dateFormat(.unix)
        ...
        struct CabinItem : ALSwiftyJSONAble{
            ...
            /// 计算仓位类型
            var type:String {
                return id.characters.last?.description ?? ""
            }
            init(jsonData:JSON){
                self.id = jsonData["id"].stringValue
                ...
                self.isProtocol = jsonData["protocol"].boolValue
            }
        }
        /// 企业新老版航段信息
        struct LegItem:ALSwiftyJSONAble{
            ...
            init(jsonData:JSON){
                ...
                self.marketAirlineShort = jsonData["flightShortName"].stringValue
...
extension Array where Element == CoFlightSearchResult.FlightItem {
    /// 航班排序
    ///
    /// - Parameter by: 排序规则
    /// - Returns: 排序后的结果
    ...
    func filterCompany(_ companyArr:[String]) -> [Element]{
        return self.filter{companyArr.contains($0.legList.first?.marketFlightName ?? "unknow")}
    }
    /// 根据仓位过滤航班信息
    ///
