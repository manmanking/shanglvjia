...
import SwiftDate
/// 企业新版航班查询结果
struct CoNewFlightSearchResult:ALSwiftyJSONAble {
    /// 起飞城市名
    ...
    init(jsonData result:JSON){
        ...
        self.departTakeoffDate = result["takeOffDate"].dateFormat(.unix) ?? DateInRegion()
...
extension Array where Element == CoNewFlightSearchResult.FlightItem {
    /// 航班排序
    ///
    /// - Parameter by: 排序规则
    /// - Returns: 排序后的结果
