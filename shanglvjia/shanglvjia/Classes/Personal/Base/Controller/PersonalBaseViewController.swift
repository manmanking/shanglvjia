...
//        self.navigationController?.pushViewController(companyAccountView, animated: true)
    ...
    func checkInstallMapType(latitude:String,longitude:String,distionName:String) {
        guard latitude.isEmpty == false && longitude.isEmpty == false else {
            return
        }
        ...
        let destionCoordinate:CLLocationCoordinate2D = CLLocationCoordinate2D.init(latitude: Double(latitude)!, longitude: Double(longitude)!)
...
//                return
        ...
        if baidumap {
            ...
            let qqAction:UIAlertAction = UIAlertAction.init(title: "百度地图", style: UIAlertActionStyle.default) { (action) in
                let hotelMapDestination:String = "baidumap://map/geocoder?location=" + latitude + "," + longitude + "&coord_type=gcj02&src=" + "com.tbi.shanglvjia.debug"
                
                weakSelf?.hotel2Map(hotelMap2Geo: hotelMapDestination)
                return
               
                let firstBaseInfoStr:String = "baidumap://map/direction?origin={{我的位置}}&"
                let coorde:String = "destination=latlng:\((latitude)),\((longitude))|name=\((distionName ))"
        ...
        if gaodemap {
            ...
            let qqAction:UIAlertAction = UIAlertAction.init(title: "高德地图", style: UIAlertActionStyle.default) { (action) in
                let hotelMapDestination = "iosamap://viewMap?sourceApplication=途途帮&poiname=" + distionName + "&lat=" + latitude + "&lon=" + longitude + "&dev=1"
                weakSelf?.hotel2Map(hotelMap2Geo: hotelMapDestination)
                return
                
                let firstBaseInfoStr:String = "iosamap://navi?sourceApplication=com.tbi.shanglvjia.tutubangs&backScheme=iosamap://"
                let coorde:String = "&lat=" + (latitude) + "&lon=" + (longitude)
...
//                return
                ...
                let coorde:String = "&saddr=&daddr=" + (latitude) + "," + (longitude)
        ...
        if qqmap {
            ...
            let qqAction:UIAlertAction = UIAlertAction.init(title: "腾讯地图", style: UIAlertActionStyle.default) { (action) in
                ...
                let hotelMapDestination:String = "https://apis.map.qq.com/tools/poimarker?type=0&marker=coord:" + latitude + "," + longitude + ";title:" + distionName + "&key=OB4BZ-D4W3U-B7VVO-4PJWW-6TKDJ-WPB77&referer=途途帮"
                ...
                let coorde:String = "&tocoord=" + (latitude) + "," + (longitude)
