...
class PersonalFlightOrderViewController: PersonalBaseViewController,PayManagerStateProtocol,UITableViewDelegate,UITableViewDataSource {
   ...
   // private var specialFlightTrip:[PSepcailFlightCabinModel.ResponsesListVo] = Array()
    ...
    public var billTypeArr : [String] = Array()
...
//        footerView.totalPriceLabel.text = orderModel.totalRate
    ...
    func caculateAmount()  {
        ...
        if insuranceStatus == true {
            ...
            if selectedPassengerArr.count > 0 {
                amountPrices += Float(selectedPassengerArr.count * 20 * tmpInsuranceNum)
                pricesDetailArr.append(("保险", "¥20 X  \((selectedPassengerArr.count * tmpInsuranceNum).description)"))
            }else{
                amountPrices += 20
                insuranceNum = tmpInsuranceNum
                pricesDetailArr.append(("保险","¥20 X \(tmpInsuranceNum)"))
            }
        }
...
//            }
            ...
            if childArr.count > 0 {
                ...
                pricesDetailArr.append(("儿童", "¥\(onsaleFlightTrip.last?.cabins[onsaleFlightTrip.last?.selectedCabinIndex ?? 0].childPrice.floatValue.TwoOfTheEffectiveFraction() ?? "0") X \(childArr.count) "))
                pricesDetailArr.append(("基建", "¥\(onsaleFlightTrip.last?.cabins[onsaleFlightTrip.last?.selectedCabinIndex ?? 0].childTax.floatValue.TwoOfTheEffectiveFraction() ?? "0") X \(childArr.count)"))
                pricesDetailArr.append(("燃油税","¥\(onsaleFlightTrip.last?.cabins[onsaleFlightTrip.last?.selectedCabinIndex ?? 0].childFuelTax.floatValue.TwoOfTheEffectiveFraction() ?? "0") X \(childArr.count)"))
            }
//            if myselfContain {
...
//            }
            ...
            if ramainPassenger > 0 {
                ...
                pricesDetailArr.append(("成人", "¥\(onsaleFlightTrip.last?.cabins[onsaleFlightTrip.last?.selectedCabinIndex ?? 0].price.floatValue.TwoOfTheEffectiveFraction() ?? "0") X \(ramainPassenger)"))
...
//            }
            ...
            for (index,element) in onsaleFlightTrip.enumerated() {
                ...
                if childArr.count > 0 {
                    ...
                    pricesDetailArr.append(("儿童" + flightTripType, "¥\(element.cabins[element.selectedCabinIndex ?? 0].childPrice.floatValue.TwoOfTheEffectiveFraction()) X \(childArr.count) ")) //"票价(儿童)"
                    pricesDetailArr.append(("基建" + flightTripType, "¥\(element.cabins[element.selectedCabinIndex ?? 0].childTax.floatValue.TwoOfTheEffectiveFraction()) X \(childArr.count)")) //基建(儿童)
                    pricesDetailArr.append(("燃油税" + flightTripType,"¥\(element.cabins[element.selectedCabinIndex ?? 0].childFuelTax.floatValue.TwoOfTheEffectiveFraction()) X \(childArr.count)")) //燃油税(儿童)
                }
//                if myselfContain {
...
//                }
                ...
                if ramainPassenger > 0 {
                    ...
                    pricesDetailArr.append(("成人" + flightTripType, "¥\(element.cabins[element.selectedCabinIndex ?? 0].price.floatValue.TwoOfTheEffectiveFraction()) X \(ramainPassenger)")) //"票价"
...
//            }
            ...
            for (index,element) in commonFlightTrip.enumerated() {
                ...
                if childArr.count > 0 {
                    ...
                    amountPrices += (element.cabins[element.selectedCabinIndex ?? 0].childFuelTax.floatValue * Float(childArr.count))
                    pricesDetailArr.append(("儿童" + flightTripType, "¥\(element.cabins[element.selectedCabinIndex ?? 0].childPrice.floatValue.TwoOfTheEffectiveFraction()) X \(childArr.count) ")) //票价(儿童)
                    pricesDetailArr.append(("基建" + flightTripType, "¥\(element.cabins[element.selectedCabinIndex ?? 0].childTax.floatValue.TwoOfTheEffectiveFraction()) X \(childArr.count)"))
                    pricesDetailArr.append(("燃油税" + flightTripType,"¥\(element.cabins[element.selectedCabinIndex ?? 0].childFuelTax.floatValue.TwoOfTheEffectiveFraction()) X \(childArr.count)"))
                }
                if myselfContain {
                    amountPrices += element.cabins[element.selectedCabinIndex ?? 0].price.floatValue
                    pricesDetailArr.append(("本人" + flightTripType, "¥" +   (element.cabins[element.selectedCabinIndex ?? 0].price.floatValue.TwoOfTheEffectiveFraction())))
                    amountPrices += element.cabins[element.selectedCabinIndex ?? 0].tax.floatValue
                    amountPrices += element.cabins[element.selectedCabinIndex ?? 0].fuelTax.floatValue
                    pricesDetailArr.append(("基建" + flightTripType, "¥" +   (element.cabins[element.selectedCabinIndex ?? 0].tax.floatValue.TwoOfTheEffectiveFraction())))
                    pricesDetailArr.append(("燃油税" + flightTripType, "¥" +   (element.cabins[element.selectedCabinIndex ?? 0].fuelTax.floatValue.TwoOfTheEffectiveFraction())))
                ...
                if ramainPassenger > 0 {
                    ...
                    pricesDetailArr.append(("家属" + flightTripType, "¥\(price.TwoOfTheEffectiveFraction()) X \(ramainPassenger)"))
                    amountPrices += (element.cabins[element.selectedCabinIndex ?? 0].tax.floatValue * Float(ramainPassenger))
                    pricesDetailArr.append(("基建" + flightTripType, "¥\(element.cabins[element.selectedCabinIndex ?? 0].tax.floatValue.TwoOfTheEffectiveFraction()) X \(ramainPassenger)"))
                    amountPrices += (element.cabins[element.selectedCabinIndex ?? 0].fuelTax.floatValue * Float(ramainPassenger))
                    pricesDetailArr.append(("燃油税" + flightTripType,"¥\(element.cabins[element.selectedCabinIndex ?? 0].fuelTax.floatValue.TwoOfTheEffectiveFraction()) X \(ramainPassenger)"))
    ...
    func submitOrder() {
        ...
        if invoiceStatus {
            if billTypeStr == "行程单"
            {
                ...
                if billTypeStr == "电子发票"{
                    invoiceInfo.exKind = "0"
                    invoiceInfo.address = ""
                }else{
                    invoiceInfo.exKind = "1"
                    invoiceInfo.address = addressCity.value + addressStreet.value
                }
            }
...
//        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 1 {
            ...
            passengerInfoCell.setCell(leftStr: "第\(indexPath.row)人", rightStr:tmpname)
        ...
        if indexPath.section == 3 + warmTipDefaultSection {
            ...
            cell.selectionStyle = UITableViewCellSelectionStyle.none
            cell.fillDataSources(title: "意外险", content: "20元/位x \(insuranceNum)份", isOpen: insuranceStatus, indexPathCell: indexPath.row)
            printDebugLog(message: "这是 计算保险 cell")
            printDebugLog(message: "\(insuranceNum)")
            cell.personalFlightInvoiceTableViewBlock = { selectedStatus,_ in
                weakSelf?.modifyInsurance(selectedStatus: selectedStatus)
            }
            return cell
        }
        if indexPath.section == 4 + warmTipDefaultSection {
            ...
            if billTypeStr == "行程单"{
                ...
                cell.fillCell(isBill:invoiceStatus,type:invoiceTypeIndex,billName:"机票",billType:billTypeStr,typeArr:billTypeArr)
                ...
                cell.billTypeReturnBlock = { typeStr in
...
//        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 3 + warmTipDefaultSection {
            popPersonalNewAlertView(content:"保险",titleStr:"保险内容",btnTitle:"确定")
        }
    ...
    func modifyInsurance(selectedStatus:Bool) {
