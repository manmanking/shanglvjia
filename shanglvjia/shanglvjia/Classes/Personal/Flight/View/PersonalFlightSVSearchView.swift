...
class PersonalFlightSVSearchView: UIView ,UITextFieldDelegate,UITableViewDataSource,UITableViewDelegate{
    ...
    typealias ChildDescriptionBlock = () ->Void
    
    public var childDescriptionBlock:ChildDescriptionBlock!
...
//    private let  moreRoundTripLabel:UILabel = UILabel()
    ...
    /// 携带婴儿
    private let childBaseBackgroundView:UIView = UIView()
    private let childButton:UIButton = UIButton.init(title: " 携带儿童", titleColor: PersonalThemeMajorTextColor, titleSize: 16)
    private let babyButton:UIButton = UIButton.init(title: " 携带婴儿", titleColor: PersonalThemeMajorTextColor, titleSize: 16)
    private let childDesButton:UIButton = UIButton.init(title: "儿童婴儿说明", titleColor: PersonalThemeMinorTextColor, titleSize: 12)
    ...
    private func setUIViewAutolayout() {
        ...
        ///携带儿童
        subBaseBackgroundView.addSubview(childBaseBackgroundView)
        childBaseBackgroundView.snp.makeConstraints { (make) in
            make.top.equalTo(dayBaseBackgroundView.snp.bottom)
            make.left.right.equalToSuperview()
            make.height.equalTo(60)
        }
        setChildBackgroundViewAutolayout()
        ...
        searchButton.snp.remakeConstraints { (make) in
            make.top.equalTo(childBaseBackgroundView.snp.bottom).offset(30)
    ...
    func setTableViewAutolayout() {
        ...
        moreTripTableView.snp.makeConstraints { (make) in
            make.top.equalToSuperview().inset(45)
            make.left.right.equalToSuperview()
            make.height.equalTo( 120 * tableViewDataSources.count + 80 + tableViewAddCellHeight)
        }
    }
...
//        }
    ...
    private func setAddressBaseBackgroundViewAutolayout() {
        ...
        flightStartPointTextField.font = UIFont.systemFont(ofSize: 20)
        ...
        flightEndPointTextField.font = UIFont.systemFont(ofSize: 20)
    ...
    private func setDayBaseBackgroundViewAutolayout() {
        ...
        startDayLabel.font = UIFont.systemFont(ofSize: 18)
        ...
        startDayWeekLabel.font = UIFont.systemFont(ofSize: 12)
    ...
    private func setDaySubbaseRightDateBackgroundViewAutolayout() {
        endDayWeekLabel.font = UIFont.systemFont(ofSize: 12)
        ...
        endDayLebl.font = UIFont.systemFont(ofSize: 18)
    ...
    /// 设置返程的日期
    private func setChildBackgroundViewAutolayout() {
        childBaseBackgroundView.addSubview(childButton)
        childButton.setImage(UIImage(named:"visa_pay_noselect"), for: UIControlState.normal)
        childButton.setImage(UIImage(named:"visa_pay_select"), for: UIControlState.selected)
        childButton.snp.makeConstraints { (make) in
            make.left.equalToSuperview()
            make.width.equalToSuperview().dividedBy(3)
            make.top.bottom.equalToSuperview()
        }
        childButton.addTarget(self, action: #selector(childButtonClick(sender:)), for: UIControlEvents.touchUpInside)
        
        childBaseBackgroundView.addSubview(babyButton)
        babyButton.setImage(UIImage(named:"visa_pay_noselect"), for: UIControlState.normal)
        babyButton.setImage(UIImage(named:"visa_pay_select"), for: UIControlState.selected)
        babyButton.snp.makeConstraints { (make) in
            make.left.equalTo(childButton.snp.right)
            make.width.equalToSuperview().dividedBy(3)
            make.top.bottom.equalToSuperview()
        }
        babyButton.addTarget(self, action: #selector(babyButtonClick(sender:)), for: UIControlEvents.touchUpInside)
        
        childBaseBackgroundView.addSubview(childDesButton)
        childDesButton.setImage(UIImage(named:"ic_hotel_remark"), for: UIControlState.normal)
        childDesButton.snp.makeConstraints { (make) in
            make.left.equalTo(babyButton.snp.right)
            make.width.equalToSuperview().dividedBy(3)
            make.top.bottom.equalToSuperview()
        }
        childDesButton.addTarget(self, action: #selector(childDesButtonClick), for: UIControlEvents.touchUpInside)
        let bottomLine:UILabel = UILabel()
        bottomLine.backgroundColor = TBIThemeGrayLineColor
        childBaseBackgroundView.addSubview(bottomLine)
        bottomLine.snp.makeConstraints{(make) in
            make.bottom.equalToSuperview().inset(1)
            make.height.equalTo(0.5)
            make.left.right.equalToSuperview().inset(20)
    ...
    func childButtonClick(sender:UIButton){
        sender.isSelected = !sender.isSelected
    }
    func babyButtonClick(sender:UIButton){
        sender.isSelected = !sender.isSelected
    }
    func childDesButtonClick(){
        if childDescriptionBlock != nil{
            childDescriptionBlock()
        }
    }
...
//                weakSelf?.moreRoundTripLabel.textColor = TBIThemePrimaryTextColor
    ...
    public func fillMoreTripDataSources(dataSoureces:[FlightSVSearchConditionModel]) {
        ...
        moreTripTableView.snp.remakeConstraints({ (remake) in
            ...
            remake.height.equalTo( 120 * tableViewDataSources.count + 80 + tableViewAddCellHeight)
        })
