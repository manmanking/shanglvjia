...
extension PHotelSubmitViewController:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0{
            ...
            cell.fillCell(isBill:isBill,type:typeIndex,billName:"代办酒店",billType:billTypeStr,typeArr:["电子发票","纸质发票"])
...
extension PHotelSubmitViewController{
    ...
    func nextOrder(sender: UIButton)
    {
        ...
        if isBill == true && typeIndex == 0 && personalName.value.isEmpty == true {
        ...
        if isBill == true && typeIndex == 0 && personalName.value.isEmpty == false {
            expense.exPersonName = personalName.value
            expense.exType = "0"
        }
        if isBill == true && typeIndex == 1  {
