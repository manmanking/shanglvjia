...
//    }()
    ...
    func setUIViewAutolayout() {
        ...
        mainTable.register(PersonalMainImageCell.self, forCellReuseIdentifier: "PersonalMainImageCell")
...
extension PersonalMainHomeViewController:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0{
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "PersonalMainImageCell",for: indexPath) as! PersonalMainImageCell
            cell.fillDataSources(imageName: bottomArr[indexPath.row-2]["imgName"]!,title:bottomArr[indexPath.row-2]["title"]!,des:bottomArr[indexPath.row-2]["desc"]!)
            return cell
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.row == 2{
            let detailVC:PersonalTopicDetailViewController = PersonalTopicDetailViewController()
            detailVC.nationStr = "subject"
            self.navigationController?.pushViewController(detailVC, animated: true)
        }
        if indexPath.row == 3{
             let detailVC:PersonalTopicDetailViewController = PersonalTopicDetailViewController()
            detailVC.nationStr = "japan"
            self.navigationController?.pushViewController(detailVC, animated: true)
        }
