...
class VisaBillCell: UITableViewCell,UITextFieldDelegate {
    ...
    ///发票类型
    typealias BillTypeReturnBlock = (String)->Void
    ...
    ///定投机票：电子发票0，纸质发票1
    ///特价及BSP机票：行程单
    func fillCell(isBill:Bool,type:NSInteger,billName:String,billType:String,typeArr:[String]){
    ...
    func billTopView(type:NSInteger){
        let leftArr:[String] = ["发票种类","发票内容","发票类型"]
    ...
    func selectBillsType(tap:UITapGestureRecognizer){
        ...
        roleView.finderViewSelectedResultBlock = { (cellIndex) in
            ...
            {
               weakSelf?.billTypeReturnBlock((weakSelf?.typeTitleArr[cellIndex])!)
