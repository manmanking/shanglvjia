...
//NSString * const TBICalendarDefaultCellReuseIdentifier = @"TBICalendarDefaultCellReuseIdentifier";
...
{   
    ...
    // 展示的时间范围 开始的时间 和结束的时间
    _minimumDate = [self.formatter dateFromString:@"2017-01-01"];
    _maximumDate = [self.formatter dateFromString:@"2020-12-31"];
    
    _headerHeight     = FSCalendarAutomaticDimension;
    _weekdayHeight    = FSCalendarAutomaticDimension;
    _rowHeight        = FSCalendarStandardRowHeight*MAX(2, FSCalendarDeviceIsIPad*1.5);
    ...
    [collectionView registerClass:[TBICollectionViewCell class] forCellWithReuseIdentifier:TBICalendarDefaultCellReuseIdentifier];
    if (self.selectedDates.count>1) {
        [self addDataSource:self.selectedDates andFromDate:self.selectedDates[0] andToDate:self.selectedDates[1]];    
    }
    // end of line
...
- (NSInteger)numberOfSectionsInCollectionView:(UICollectionView *)collectionView
{
    ...
    NSLog(@"collection section %ld",(long)self.calculator.numberOfSections);
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    if (self.floatingMode) {
        ...
        NSLog(@"collection  section %ld row %ld",section , numberOfRows * 7);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSLog(@"item show time  %@,index: section %ld, row:%ld",date,(long)indexPath.section,(long)indexPath.row);
    // 现在时间
    NSDate *currentDate = [NSDate date];
    NSDateFormatter *formatter = [[NSDateFormatter alloc]init];
    NSString *showDayStr = @"";
    
     TBICollectionViewCell*cell = [self.collectionView dequeueReusableCellWithReuseIdentifier:TBICalendarDefaultCellReuseIdentifier forIndexPath:indexPath];
    if (cell == nil) {
        cell = [[TBICollectionViewCell alloc]init];
        
    }
    cell.isSelected = false;
    cell.isToday = false;
    
    NSString *showFirstTitleStr = @"去程";
    if (self.showTitleArr.count >= 1) {
        showFirstTitleStr = self.showTitleArr[0];
    }
    NSString *showSecondTitleStr = @"返程";
    if (self.showTitleArr.count >= 2) {
        showSecondTitleStr = self.showTitleArr[1];
    }
    ...
    switch (cell.monthPosition) {
        case TBICalendarMonthPositionPrevious:
            cell.isActivity = false;
            NSLog(@"上一个月 date:%@,indexsection :%ld,row:%ld",date,indexPath.section,indexPath.row);
            [cell setCellTypeNone];
            return cell;

        case TBICalendarMonthPositionCurrent:
            
            [formatter setDateFormat:@"yyyy"];
            NSInteger showYear=[[formatter stringFromDate:date] integerValue];
            NSInteger currentYear = [[formatter stringFromDate:currentDate] integerValue];
            [formatter setDateFormat:@"MM"];
            NSInteger  showMonth=[[formatter stringFromDate:date] integerValue];
            NSInteger currentMonth = [[formatter stringFromDate:currentDate] integerValue];
            [formatter setDateFormat:@"dd"];

            NSInteger showDay= [[formatter stringFromDate:date] integerValue]; //integerValue];
            NSInteger currentDay = [[formatter stringFromDate:currentDate] integerValue];
            
            
            showDayStr = [NSString stringWithFormat:@"%ld",(long)showDay];
            
            NSLog(@"本月时间 date:  %@,index: section %ld, row:%ld",date,(long)indexPath.section,(long)indexPath.row);
            ...
            if (showYear == currentYear && showMonth == currentMonth && showDay == currentDay) {
                cell.isToday = true;
            }else
            {
                cell.isToday = false;
            ...
            if (self.selectedDates.count == 1) {
                if ([self.selectedDates[0]compare:date] == NSOrderedSame) {
                    // 设置去程 页面布局
                   // NSLog(@"去程 页面布局");
                    
                  [cell configAppearanceTitle:showDayStr andSubtitleStr:showFirstTitleStr andSubbackgroundAlpha:1.0];
                    return cell;
                }
            }
            if (self.selectedDates.count >= 2)
            {
                ...
                if ([self.selectedDates[1] compare:date] == NSOrderedSame) {
                    [cell configAppearanceTitle:showDayStr andSubtitleStr:showSecondTitleStr andSubbackgroundAlpha:1.0];
                    return cell;
                }
                
                if ([self.selectedDates containsObject:date]) {
                    [cell configAppearanceTitle:showDayStr andSubtitleStr:@"" andSubbackgroundAlpha:0.2];
                    return cell;
                }
                
            }
            
            //NSLog(@" the day is nice day  ...");
            [cell configAppearanceTitle:showDayStr andSubtitleStr:@"" andSubbackgroundAlpha:1.0];
           
            return cell;

        case TBICalendarMonthPositionNext:
            NSLog(@"下一个月 show time  %@,index: section %ld, row:%ld",date,(long)indexPath.section,(long)indexPath.row);
            cell.isActivity = false;
            [cell setCellTypeNone];
            return cell;
        default:
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TBICollectionViewCell *cell = [collectionView cellForItemAtIndexPath:indexPath];
    NSDate *selectedDate = [self.calculator dateForIndexPath:indexPath];
    NSLog(@"did selected %@",selectedDate);
    
    cell.monthPosition = [self.calculator monthPositionForIndexPath:indexPath];
    
    if (cell.monthPosition != TBICalendarMonthPositionCurrent || cell.isActivity == false) {
        return;
    }
    ...
    if (self.selectedDates.count == 0 ) {
        ...
        self.selectedDateBlock(self.selectedDates);
        }
        return;
    ...
    if (self.selectedDates.count == 1) {
        ...
        if ([self.selectedDates[0] compare:selectedDate] == NSOrderedAscending) {
            [self.selectedDates addObject:selectedDate];
            [self addDataSource:self.selectedDates andFromDate:self.selectedDates[0] andToDate:selectedDate];
            [self.collectionView reloadData];
        ...
        {
            [self.selectedDates replaceObjectAtIndex:0 withObject:selectedDate];
            [self.collectionView reloadData];
            return;
...
- (void)addDataSource:(NSMutableArray*)datasources andFromDate:(NSDate *) fromDate andToDate:(NSDate *) toDate
{
    NSDate *nextDate = [fromDate dateByAddingTimeInterval:24*60*60];
    while ([nextDate compare:toDate] == NSOrderedAscending) {
        [datasources addObject:nextDate];
...
- (NSArray *)calculateSelectedDate:(NSArray *)datasourcesArr toIndexPath:(NSIndexPath *) toIndexPath
{
    NSMutableArray *resultMutableArr = [NSMutableArray arrayWithCapacity:5];
    if (datasourcesArr.count < 2) {
        [resultMutableArr addObject:[self.calculator indexPathForDate:datasourcesArr[0]]];
        NSArray *returnArr = [NSArray arrayWithArray:resultMutableArr];
        return returnArr;
    }
    
    
    
    NSDate *fromDate = datasourcesArr[0];
    NSDate *toDate = datasourcesArr[1];
    
    [resultMutableArr addObject:toIndexPath];
    
    for (int i = 1; i < datasourcesArr.count; i++) {
        if ([fromDate compare:datasourcesArr[datasourcesArr.count - i]] == NSOrderedAscending &&
            [toDate compare:datasourcesArr[datasourcesArr.count - i]] == NSOrderedDescending)
        {
            NSIndexPath *tmpIndexPath = [self.calculator indexPathForDate:datasourcesArr[datasourcesArr.count - i]];
            [resultMutableArr addObject:tmpIndexPath];
        }
    }
...
- (NSInteger)monthOfDate:(NSDate *)date andFormat:(NSString *)formatStr
{
    NSDateFormatter *formatter = [[NSDateFormatter alloc]init];
...
-(NSInteger)yearOfDate:(NSDate *)date andFormat:(NSString *)formatStr
{
    NSDateFormatter *formatter = [[NSDateFormatter alloc]init];
    [formatter setDateFormat:formatStr];
    return [[formatter stringFromDate:date] integerValue];
...
-(NSInteger)dayOfDate:(NSDate *)date andFomat:(NSString *)formatStr
{
    NSDateFormatter *formatter = [[NSDateFormatter alloc]init];
    [formatter setDateFormat:formatStr];
    return [[formatter stringFromDate:date] integerValue];
...
- (BOOL)requestBoundingDatesIfNecessary
{
    if (_needsRequestingBoundingDates) {
        ...
        NSDate *newMin = [self.dataSourceProxy minimumDateForCalendar:self]?:[self.formatter dateFromString:@"2017-01-01"];
        newMin = [self.gregorian dateBySettingHour:0 minute:0 second:0 ofDate:newMin options:0];
        NSDate *newMax = [self.dataSourceProxy maximumDateForCalendar:self]?:[self.formatter dateFromString:@"2020-12-31"];
