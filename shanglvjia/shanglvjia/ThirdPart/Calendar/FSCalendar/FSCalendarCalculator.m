...
{
    return [self indexPathForDate:date atMonthPosition:FSCalendarMonthPositionCurrent scope:FSCalendarScopeMonth];
}
...
- (NSIndexPath *)indexPathForDate:(NSDate *)date atMonthPosition:(FSCalendarMonthPosition)position scope:(FSCalendarScope)scope
{
    ...
    NSLog(@" test  date:%@ indexPath:section:%ld row%ld",date,indexPath.section,indexPath.item);
    return indexPath;
}

- (NSIndexPath *)indexPathForDate:(NSDate *)date atMonthPosition:(FSCalendarMonthPosition)position
{
    NSLog(@"indexPathFordate:%@   position:%lu",date,(unsigned long)position);
...
//        一个月共有 多少天
        ...
        NSLog(@"numberOfDaysInMonth%ld",numberOfDaysInMonth);
        NSLog(@"numberOfPlaceholdersForPrev%ld",numberOfPlaceholdersForPrev);

        NSLog(@"month:%@   rowCount%@",month,rowCount);
...
- (FSCalendarMonthPosition)monthPositionForIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDate *date = [self dateForIndexPath:indexPath];//获取item 展示时间
