...
{
    static var defaultFirstDate =  Date()
    static var defaultLastDate =  Date() + 30.month
    /**
     *日历开始的日期
     */
    ...
    open func dayState(_ indexPath:IndexPath) -> (Date,DayStateOptions){
        let monthInfo =  getMonthInfo((indexPath as NSIndexPath).section + monthSection)
...
public struct DayStateOptions:OptionSet{
    ...
    //售罄
    public static var SoldOut:DayStateOptions{
        return DayStateOptions(rawValue: 1<<6)
    }
    //过期
    public static var Overdue:DayStateOptions{
        return DayStateOptions(rawValue: 1<<7)
    }
