...
class HotelSearchConditionViewController: CompanyBaseViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    private var lowerStr:String = ""
    private var highStr:String = ""
    ...
    override func viewDidLoad() {
        ...
        let paraModel = HotelManager.shareInstance.searchParameterUserDraw()
        lowerStr = paraModel.lowStr
        highStr = paraModel.hightStr
        currentSearchConditionCategoryRecommondIndex = paraModel.recommendIndex
        currentSearchConditionCategoryClassIndex = paraModel.levelIndex
        currentSearchConditionCategoryBrandIndex = paraModel.styleIndex
        setUIViewAutolayout()
...
//            hotelBrandBackgroundView.isHidden = true
            ...
            break
        case 1:
            if HotelManager.shareInstance.searchConditionCityCategoryRegionDraw()
                .cityBrandInfos?.forthClass?.isEmpty == false{
                //1是 五星 豪华
    ...
    func searchButtonAction() {
        ...
        searchCondition.brandId = tableViewDataSourcesBrand[currentSearchConditionCategoryBrandIndex].brandElongId
        searchCondition.score = tableViewDataSourcesRecommend[currentSearchConditionCategoryRecommondIndex].content
        searchCondition.hotelStarRate = tableViewDataSourcesClass[currentSearchConditionCategoryClassIndex].content
        HotelManager.shareInstance.searchConditionUserStore(searchCondition: searchCondition)
        let paraModel = HotelManager.shareInstance.searchParameterUserDraw()
        paraModel.lowStr = lowerStr
        paraModel.hightStr = highStr
        paraModel.recommendIndex = currentSearchConditionCategoryRecommondIndex
        paraModel.levelIndex = currentSearchConditionCategoryClassIndex
        paraModel.styleIndex = currentSearchConditionCategoryBrandIndex
    ...
    override func rightButtonAction(sender: UIButton) {
        ...
        searchCondition.score = ""
         currentSearchConditionCategoryRecommondIndex = 0
         currentSearchConditionCategoryClassIndex = 0
         currentSearchConditionCategoryBrandIndex  = 0
