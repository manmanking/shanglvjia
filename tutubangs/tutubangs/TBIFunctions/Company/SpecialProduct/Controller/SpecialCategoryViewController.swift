...
class SpecialCategoryViewController: CompanyBaseViewController {
    ...
    fileprivate var sectionHeaderTitle:[[String:String]] = []
    
    override func viewDidLoad() {
        super.viewDidLoad()
        setNavigation(title:"特价产品预订")
        initData()
...
extension SpecialCategoryViewController {
    
    func initData(){
        showLoadingView()
        SpecialProductService.sharedInstance.categorys(travelItem?.productId ?? "").subscribe{ event in
...
//        }
    ...
    func nextStep(sender: UIButton) {
        if self.islogin() {
            ...
            let vc = SpecialOrderViewController()
...
extension SpecialCategoryViewController: UICollectionViewDelegate, UICollectionViewDataSource{
    ...
    func updateProductCollectionView(){
        ...
        //self.initPrice(id:"103",dateMonth:saleDate)
        initView()
    }
    ...
    func initPrice (id:String, dateMonth:String){
        ...
        showLoadingView()
        SpecialProductService.sharedInstance.searchPrice(id: travelItem?.productId ?? "1", categoryId: id, form).subscribe{ event in
            self.hideLoadingView()
            if case .next(let e) = event {
                self.data = e
    ...
    func refreshTableView(data:SpecialPriceListItem?){
        ...
        let type = travelItem?.productType ?? .travel
        // 0  显示单价   1显示 成人 儿童(占床) 儿童(不占床)
        var priceTitleType:Int = 0
        // 0 房间价  1 显示单房差  2车辆价格
        var singleRoomDifferenceType:Int = 0
        switch type {
        case .airCar:
            sectionHeaderTitle.append(["title":"选择机票数量"])
            sectionHeaderTitle.append(["title":"选择车辆数"])
            priceTitleType = 0
            singleRoomDifferenceType = 2
        case .airTicket:
            sectionHeaderTitle.append(["title":"选择机票数量"])
            sectionHeaderTitle.append(["title":"选择数量"])
            priceTitleType = 0
            singleRoomDifferenceType = 0
        case .airVisa:
            sectionHeaderTitle.append(["title":"选择数量"])
            sectionHeaderTitle.append(["title":"选择数量"])
            priceTitleType = 0
        ...
        case .holiday:
            sectionHeaderTitle.append(["title":"选择数量"])
            priceTitleType = 1
            singleRoomDifferenceType = 1
        case .travel:
            sectionHeaderTitle.append(["title":"选择数量"])
            priceTitleType = 1
            singleRoomDifferenceType = 1
        case .hotelAdmission:
            sectionHeaderTitle.append(["title":"选择门票数量"])
            sectionHeaderTitle.append(["title":"选择房间数量"])
            priceTitleType = 0
            singleRoomDifferenceType = 0
        case .hotelTicket:
            sectionHeaderTitle.append(["title":"选择餐券数量"])
            sectionHeaderTitle.append(["title":"选择房间数量"])
            priceTitleType = 0
            singleRoomDifferenceType = 0
        case .airHotel:
        ...
        if data?.adultPrice != 0 && data?.adultPrice != nil{ //成人价格
            if priceTitleType == 0 {
                 tableSectionOneData.append(["title":"单价","type":PriceType.adultPrice,"price":data?.adultPrice ?? 0,"number":data?.stock == 0 ? 0 : 1])
            }else {
        ...
        if data?.childNobedPrice != 0 && data?.childBedPrice != nil{//儿童不占床
            if priceTitleType == 0 {
                 tableSectionOneData.append(["title":"儿童","type":PriceType.childBedPrice,"price":data?.childNobedPrice ?? 0,"number":0])
            }else {
        ...
        if data?.singleRoomDifference != 0 && data?.singleRoomDifference != nil{//单房差
            if singleRoomDifferenceType == 0{
                 tableSectionTwoData.append(["title":"房间价","type":PriceType.singleRoomDifference,"price":data?.singleRoomDifference ?? 0,"number":0])
            }else if singleRoomDifferenceType == 1{
                 tableSectionTwoData.append(["title":"单房差","type":PriceType.singleRoomDifference,"price":data?.singleRoomDifference ?? 0,"number":0])
            }else if singleRoomDifferenceType == 2{
                 tableSectionTwoData.append(["title":"车辆单价","type":PriceType.singleRoomDifference,"price":data?.singleRoomDifference ?? 0,"number":0])
            }
    ...
    func initCollectionView(){
        ...
        collectionView?.snp.makeConstraints({ (make) in
            make.width.equalTo(ScreenWindowWidth-30)
    ...
    func nextMonth(sender: UIButton) {
        ...
        let saleDate = DateInRegion(absoluteDate: date).string(custom: "yyyy-MM")
        self.dateHeaderView.dayLabel.text = DateInRegion(absoluteDate: date).string(custom: "yyyy年M月")
        dateHeaderView.lastMonth.isSelected = true
        self.initPrice(id:self.categorysList?[categoryIdIndex].categoryId ?? "",dateMonth:saleDate)
...
extension SpecialCategoryViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func initTableView () {
        ...
        var height = (tableSectionOneData.count  + tableSectionTwoData.count) * 44 + 97
        if tableSectionThree == 1{
            height += 37
        }
    ...
    func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
         let flag = travelItem?.productType == .travel || travelItem?.productType == .holiday
        if  section == 1 && tableSectionTwo != 0 && flag{
            return 50
        }
        if tableSectionThree != 0 && tableSectionTwo + tableSectionThree == section{
            return 0
        }
        return 47
    }
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        headerView?.fillCell(title: (sectionHeaderTitle[section]["title"]) ?? "")
        return headerView
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            ...
            cell.fillCell(model: tableSectionOneData[indexPath.row],stock:selectData?.stock ?? 0)
            return cell
            ...
            cell.selectionStyle = .none
            cell.fillCell(model: tableSectionTwoData[indexPath.row],stock:selectData?.stock ?? 0)
            return cell
        }
