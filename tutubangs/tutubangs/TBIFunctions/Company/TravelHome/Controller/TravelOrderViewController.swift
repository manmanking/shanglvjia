//
//  TravelOrderViewController.swift
//  shop
//
...
class TravelOrderViewController: CompanyBaseViewController {
...
extension TravelOrderViewController{
    ...
    func submitClick(btn: UIButton) {
        ...
        TravelService.sharedInstance.submitTravel(travelForm).subscribe{ event in
            self.hideLoadingView()
            if case .next( let result) = event {
                if self.travelItem?.confirm ?? false{//跳到首页
                    ...
                    paymentView.productTypePayment = ProductTypePayment.Travel
...
extension TravelOrderViewController: UITableViewDataSource, UITableViewDelegate {
