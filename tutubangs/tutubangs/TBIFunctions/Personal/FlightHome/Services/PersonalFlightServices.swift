...
extension  PersonalFlightServices: Validator{
    ...
    func personalFlightInvoices(id:String,type:String) -> Observable<PersonalFlightInvoiceSuranceModel>{
        let provider = RxMoyaProvider<PersonalFlightRouter>()
        return provider
            .request(.personalFlightInvoices(productId:id,typeStr:type))
            .debugHttp(true)
            .validateJustReturn(to: PersonalFlightInvoiceSuranceModel.self)
    }
