//
//  PHoteiDetailNewViewController.swift
//  shanglvjia
//
...
class PHoteiDetailNewViewController: PersonalBaseViewController {
    
    
    // 从哪个页面过来
    public var fromWhereView:AppModelCatoryENUM = AppModelCatoryENUM.Default
    
    public var fromWhereString:String = ""
    ...
    fileprivate var detailTableView = UITableView.init(frame:CGRect.zero, style: UITableViewStyle.grouped)
    ///导航头
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(initTopView), name: NSNotification.Name.UIApplicationDidBecomeActive, object: nil)
    }
    ...
    func setTableViewHeaderViewAutolayout() -> PersonalHotelDetaiInfolHeaderView{
        ...
        return personalHotelDetaiInfolHeaderView
    }
    ...
    func fillHeaderViewDataSources() {
        ...
        case .PersonalSpecialHotel:
            if specialHotelItemInfo.hotelInfosList.isEmpty == true {
                specialHotelItemInfo.hotelInfosList = specialHotelDetailInfo.specialHotelBaseResponse.hotelInfosList
            }
    ...
    func setTableViewAutolayout() {
        ...
        detailTableView.contentInset = UIEdgeInsetsMake(520, 0, 0, 0)
        self.view.addSubview(setTableViewHeaderViewAutolayout())
    }
    ...
    func specialHotelIntoBookInfoView(selectedRoom:NSInteger,selectedRoomPlan:NSInteger) {
        ...
        for invoiceModel in specialRoomInfoList[selectedRoom].ratePlanInfoList[selectedRoomPlan].orderInvoiceBaseList{
            let model = PTravelNearbyDetailModel.InvoicesModel(jsonData: "")
            model?.name = invoiceModel.name
            model?.value = invoiceModel.value
            invoiceArr.append((model)!)
        }
    ...
    func normalHotelIntoBookInfoView(selectedRoom:NSInteger,selectedRoomPlan:NSInteger) {
        ...
        nextVC.checkInDate = checkInDate
        nextVC.checkOutDate = checkOutDate
        for invoiceModel in hotelNormalDetailInfo.hotelRoomInfoList[selectedRoom].ratePlanInfoList[selectedRoomPlan].orderInvoiceBaseList{
            let model = PTravelNearbyDetailModel.InvoicesModel(jsonData: "")
            model?.name = invoiceModel.name
            model?.value = invoiceModel.value
            invoiceArr.append((model)!)
    ...
    func showMapView() {
        ...
        switch  hotelDetailType{
        case .PersonalHotel:
            latitudeBaidu = hotelNormalDetailInfo.hotelDetailInfo.latitudeBaidu
            longitudeBaidu = hotelNormalDetailInfo.hotelDetailInfo.longitudeBaidu
            latitudeGoogle = hotelNormalDetailInfo.hotelDetailInfo.latitudeGoogle
            longitudeGoogle = hotelNormalDetailInfo.hotelDetailInfo.longitudeGoogle
            distionName = hotelNormalDetailInfo.hotelDetailInfo.hotelName
        case .PersonalSpecialHotel:
            latitudeBaidu = specialHotelDetailInfo.specialHotelBaseResponse.latitudeBaidu
            longitudeBaidu = specialHotelDetailInfo.specialHotelBaseResponse.longitudeBaidu
            latitudeGoogle = specialHotelDetailInfo.specialHotelBaseResponse.latitudeGoogle
            longitudeGoogle = specialHotelDetailInfo.specialHotelBaseResponse.longitudeGoogle
            distionName = specialHotelDetailInfo.specialHotelBaseResponse.hotelName
        default:
            break
        }
        
        mapNavigation(latitudeBaidu:latitudeBaidu , longitudeBaidu: longitudeBaidu, latitudeGoogle: latitudeGoogle, longitudeGoogle: longitudeGoogle, distionName: distionName)
    }
    /// 地图视图
    func showHotelDetailView() {
        let summaryView = PHotelMoreIntroViewController()
        ...
        switch hotelDetailType {
        case .PersonalHotel:
            let introModel = PersonalHotelIntroModel()
            ...
            introModel.distName = hotelNormalDetailInfo.hotelDetailInfo.distName
            introModel.hotelfax = hotelNormalDetailInfo.hotelDetailInfo.hotelPhone
            introModel.networkDesc = hotelNormalDetailInfo.hotelDetailInfo.networkDesc
            introModel.openTime = hotelNormalDetailInfo.hotelDetailInfo.openTime
            introModel.parkDesc = hotelNormalDetailInfo.hotelDetailInfo.parkDesc
            introModel.renovationTime = hotelNormalDetailInfo.hotelDetailInfo.renovationTime
            introModel.arivalTime = hotelNormalDetailInfo.hotelDetailInfo.arivalTime
            introModel.depTime = hotelNormalDetailInfo.hotelDetailInfo.depTime
            introModel.brandName = hotelNormalDetailInfo.hotelDetailInfo.brandName
            introModel.bussinessName = hotelNormalDetailInfo.hotelDetailInfo.bussinessName
            introModel.conferenceAmenities = hotelNormalDetailInfo.hotelDetailInfo.conferenceAmenities
            introModel.diningAmenities = hotelNormalDetailInfo.hotelDetailInfo.diningAmenities
            introModel.trafficDesc = hotelNormalDetailInfo.hotelDetailInfo.trafficDesc
            
            summaryView.introModel = introModel
        ...
        case .PersonalSpecialHotel:
            ...
            introModel.distName = specialHotelDetailInfo.specialHotelBaseResponse.distName
            introModel.hotelfax = specialHotelDetailInfo.specialHotelBaseResponse.hotelfax
            introModel.networkDesc = specialHotelDetailInfo.specialHotelBaseResponse.networkDesc
            introModel.openTime = specialHotelDetailInfo.specialHotelBaseResponse.openTime
            introModel.parkDesc = specialHotelDetailInfo.specialHotelBaseResponse.parkDesc
            introModel.renovationTime = specialHotelDetailInfo.specialHotelBaseResponse.renovationTime
            introModel.arivalTime = specialHotelDetailInfo.specialHotelBaseResponse.arivalTime
            introModel.depTime = specialHotelDetailInfo.specialHotelBaseResponse.depTime
            introModel.brandName = specialHotelDetailInfo.specialHotelBaseResponse.brandName
            introModel.bussinessName = specialHotelDetailInfo.specialHotelBaseResponse.bussinessName
            introModel.conferenceAmenities = specialHotelDetailInfo.specialHotelBaseResponse.conferenceAmenities
            introModel.diningAmenities = specialHotelDetailInfo.specialHotelBaseResponse.diningAmenities
            introModel.trafficDesc = specialHotelDetailInfo.specialHotelBaseResponse.trafficDesc
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let offsetY = scrollView.contentOffset.y + 520
        ...
        if (offsetY > 360)
        {
            personalHotelDetaiInfolHeaderView.frame =  CGRect.init(x: 0, y: kNavigationHeight - 450 , width: Int(ScreenWindowWidth), height: 520)
        }else{
            personalHotelDetaiInfolHeaderView.frame =  CGRect.init(x: 0, y: -offsetY-CGFloat(kNavigationHeight-44), width: ScreenWindowWidth, height: 520)
        }
    ...
    func personalSpecialHotelDetailInfoNET() {
        ...
        let localCheckInDate = Date().startOfDay
        let request = SpecialHotelDetailRequest()
        request.corpCode = [PersonalSpecialHotelCorpCode]
        request.endDate = (NSNumber.init(value:(localCheckInDate.startOfDay + 180.day).timeIntervalSince1970).intValue * 1000).description
        request.hotelId = specialHotelItemInfo.hotelId
        request.startDate = (NSNumber.init(value:localCheckInDate.startOfDay.timeIntervalSince1970).intValue * 1000).description
    ...
    func specialHotelFillDataSources(response:SpecialHotelDetailResponse) {
        ///printDebugLog(message: response.mj_keyValues())
        ...
        if response.hotelUsableDateInfos.isEmpty == false && fromWhereView == .PersonalSpecialHotel {
...
extension PHoteiDetailNewViewController:UITableViewDataSource,UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        sectionRoomView.roomBookButtonBlock = {roomModel in
            let nextVC:PHotelBookInfoViewController = PHotelBookInfoViewController()
            nextVC.presaleType = (roomModel.ratePlanInfoList.first?.payType)!//presaleType
            nextVC.bookMaxRoomNum = 99
            nextVC.hotelNormalDetailInfo = (weakSelf?.hotelNormalDetailInfo)!
            nextVC.hotelNormalDetailInfo.hotelDetailInfo.cityId = (weakSelf?.hotelNormalDetailInfo.hotelDetailInfo.cityId)!
            nextVC.hotelNormalDetailInfo.hotelDetailInfo.cityName = (weakSelf?.hotelNormalDetailInfo.hotelDetailInfo.cityName)!
            nextVC.bookInfoType = AppModelCatoryENUM.PersonalHotel
            nextVC.personalNormalHotelRoom = roomModel
            nextVC.personalNormalHotelRoom.selectedPlanInfoIndex = 0
            nextVC.checkInDate = (weakSelf?.checkInDate)!
            nextVC.checkOutDate = (weakSelf?.checkOutDate)!
            for invoiceModel in roomModel.ratePlanInfoList.first!.orderInvoiceBaseList{
                let model = PTravelNearbyDetailModel.InvoicesModel(jsonData: "")
                model?.name = invoiceModel.name
                model?.value = invoiceModel.value
                weakSelf?.invoiceArr.append((model)!)
            }
            nextVC.invoiceArr = (weakSelf?.invoiceArr)!
    ...
    func configNormalHotelSectionHeaderView(headerView:PersonalHotelDetailRoomRateSectionHeaderView,section:NSInteger) {
        var isProtocal:Bool = false
        var isNullRoom:Bool = true
        if hotelNormalDetailInfo.hotelRoomInfoList[section].ratePlanInfoList.count > 0{
            let item = hotelNormalDetailInfo.hotelRoomInfoList[section].ratePlanInfoList.first
            if (item?.corpCode.contains(PersonaltbiCorpCode))!{
               isProtocal = true
            }
            if (item?.corpCode.contains(PersonalftmsCorpCode))!{
                isProtocal = true
            }
            isNullRoom = false
        }
        headerView.fillNormalHotelDataSources(model: hotelNormalDetailInfo.hotelRoomInfoList[section],index: section,isProtocol:isProtocal ,isNullRoom:isNullRoom)
    }
    ...
    func specialHotelCaculateRowHeight(indexPath:IndexPath) -> CGFloat {
        ...
        let title = item.ratePlanName
        if title.getTextWidth(font: UIFont.systemFont(ofSize: 14), height: 22) > 162{
            return 70
        }
        return 50
    ...
    func configSpecialHotelCell(cell:PersonalHotelDetailRoomRateCell,indexPath:IndexPath) {
        ...
        if specialRoomInfoList.count > indexPath.section {
            ...
            cell.fillDataSources(title: item.ratePlanName, prices: averageRate, index: indexPath, productRemark:item.productRemark, isShowProtocol: cordGroup, payType: "1",status: true)
            //            weak var weakSelf = self
            //
    ...
    func configNormalHotelCell(cell:PersonalHotelDetailRoomRateCell,indexPath:IndexPath) {
        if hotelNormalDetailInfo.hotelRoomInfoList.count > 0 {
            let item = hotelNormalDetailInfo.hotelRoomInfoList[indexPath.section].ratePlanInfoList[indexPath.row]
            cell.fillDataSources(title: item.ratePlanName, prices: item.rate.TwoOfTheEffectiveFraction(), index: indexPath, productRemark:item.refundDesc, isShowProtocol: item.corpCode, payType: item.payType,status: item.status)
            //weak var weakSelf = self
    ...
    func mapNavigation(latitudeBaidu:String , longitudeBaidu: String,latitudeGoogle:String , longitudeGoogle: String, distionName: String) {
        checkInstallMapType(latitudeBaidu: latitudeBaidu, longitudeBaidu: longitudeBaidu, latitudeGoogle: latitudeGoogle, longitudeGoogle: longitudeGoogle, distionName: distionName)
    }
