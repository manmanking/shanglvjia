...
class PSpecailOfferHotelViewController:  PersonalBaseViewController {
    ...
    //http://60.28.158.166:10081/static/banner/subpage/hotel/ios/banner_hotel@3x.png
    /// 广告头
    func initHeaderView() -> UIView {
        headerView.frame = CGRect(x: 0, y: 0, width: Int(ScreenWindowWidth), height: 140 + kNavigationHeight - 64)
    ...
    func setUIViewAutolayout() {
        ...
        headerTable.snp.makeConstraints { (make) in
            ...
            make.height.equalTo((isIPhoneX ? 217 : 192)  + kNavigationHeight + 40 * 1)
        }
...
extension PSpecailOfferHotelViewController:UITableViewDataSource,UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if tableView == headerTable{
            ...
            return CGFloat(140 + kNavigationHeight - 64 + 46 + 40 + 10 + 40 * ((selectedChinaORInternationalIndex == "1" ? returnCount(count:cityChinaArr.count) : returnCount(count:cityInterArr.count))))
        }else{
    ...
    func intoNextPersonalSpecialHotelView(selectedIndex:NSInteger) {
        let detailVC:PHoteiDetailNewViewController = PHoteiDetailNewViewController()
...
extension PSpecailOfferHotelViewController{
    ...
    func selectedMainlandORInternationalCategory(selectedIndex:String) {
        ...
        headerTable.snp.remakeConstraints({ (make) in
            make.left.right.equalToSuperview()
            make.top.equalToSuperview().offset(44-kNavigationHeight)
            make.height.equalTo((isIPhoneX ? 217 : 192)  + kNavigationHeight + 40 * ((selectedChinaORInternationalIndex == "1" ? returnCount(count:cityChinaArr.count) : returnCount(count:cityInterArr.count))))
        })
        listTableView.snp.makeConstraints { (make) in
            make.left.right.bottom.equalToSuperview()
            make.top.equalTo(headerTable.snp.bottom)
