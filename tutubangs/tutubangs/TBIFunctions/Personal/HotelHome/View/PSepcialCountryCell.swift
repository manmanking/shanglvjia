...
class PSepcialCountryCell: UITableViewCell {
    ...
    private var leftImage:UIImageView = UIImageView()
    private var rightImage:UIImageView = UIImageView()
    ...
    func creatCellUI(){
        ...
        let bgView:UIView = UIView.init(frame: CGRect(x:0,y:CGFloat(140 + kNavigationHeight - 44 - 20),width:ScreenWindowWidth,height:45))
        bgView.backgroundColor = TBIThemeBackgroundViewColor
        self.addSubview(bgView)
        
        
        for i in 0...titleArr.count-1 {
            let imageView:UIImageView = UIImageView.init(frame: CGRect(x:0 + (buttonWidth*CGFloat(i)),y:-3,width:buttonWidth,height:48))
            bgView.addSubview(imageView)
            if i == 0{
                imageView.image = UIImage(named:"ic_car_pickup")
                leftImage = imageView
            }else{
                //翻转图片的方向
            ...
            titileButton.frame = CGRect(x:0 + (buttonWidth*CGFloat(i)),y:0,width:buttonWidth,height:45)
            ...
            bgView.addSubview(titileButton)
            if i == 0 { //默认 选中 国内
...
//        }
        ...
        countryView.snp.makeConstraints { (make) in
            make.left.right.equalToSuperview()
            make.top.equalTo(120 + kNavigationHeight)
...
//        }
    ...
    /// 广告头 //http://60.28.158.166:10081/static/banner/subpage/hotel/ios/banner_hotel@3x.png
    //http://60.28.158.166:10081"
    func initHeaderView() -> UIView {
        headerView.frame = CGRect(x: 0, y: 0, width: Int(ScreenWindowWidth), height: 140 + kNavigationHeight-20)
        if PersonalBannerManager.shareInstance.getPersonalHotelBannerList().isEmpty == false {
            let bannerUrl:URL = URL.init(string: PersonalBannerManager.shareInstance.getPersonalHotelBannerList().first ?? "\(BASE_URL)/static/banner/subpage/hotel/ios/banner_hotel@3x.png") as! URL
            headerView.sd_setImage(with:bannerUrl , placeholderImage: UIImage.init(named: "bg_default_travel"))
        }else {
            
            headerView.sd_setImage(with: URL.init(string:"\(BASE_URL)/static/banner/subpage/hotel/ios/banner_hotel@3x.png"))
        }
        return headerView
    }
}
