...
class PersonalTravelViewController: PersonalBaseViewController,UITableViewDelegate,UITableViewDataSource{
    ...
    private lazy var cycleScrollView: SDCycleScrollView = {
        ...
        headerView.frame = CGRect(x: 0, y:0 , width: Int(ScreenWindowWidth), height:140 - 44 + kNavigationHeight)
        headerView.imageURLStringsGroup = ["\(BASE_URL)/static/banner/subpage/travel/ios/banner_trip@3x.png"]
        return headerView
    }()
...
//        //cycleScrollView.placeholderImage = ""
        ...
        if PersonalBannerManager.shareInstance.getPersonalFlightBannerList().isEmpty == false {
            cycleScrollView.imageURLStringsGroup = PersonalBannerManager.shareInstance.getPersonalTravelBannerList()
        }
...
//        })
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.fillDataSources(model: travelList[indexPath.row],type:type)
        return cell
    }
