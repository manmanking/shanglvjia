...
extension  VisaServices: Validator{
    ...
    func getVisaDetailInvoice(productId:String) ->Observable<[VisaOrderAddResquest.InvoiceModel]>{
        let provider = RxMoyaProvider<VisaRouter>()
        return provider
            .request(.detail(productId:productId))
            .debugHttp(true)
            .validateResponse()
            .map({ (jsonData) -> [VisaOrderAddResquest.InvoiceModel] in
                return  jsonData["invoices"].arrayValue.map{VisaOrderAddResquest.InvoiceModel.init(jsonData: $0)!}
            })
    }
