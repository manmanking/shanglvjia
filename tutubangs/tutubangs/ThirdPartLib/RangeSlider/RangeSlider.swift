...
class RangeSliderTrackLayer: CALayer {
    ...
    override func draw(in ctx: CGContext) {
        ...
        ///self.contents = ((UIImage (named: "ic_price_button")?.cgImage)!);
    }
}
...
//        ctx.translateBy(x: 0.0, y: 0);
//        ctx.draw((UIImage (named: "ic_price_button")?.cgImage)!, in: CGRect(x:0.0,y:0.0,width:10,height:10))
//        // 图片
//        ctx.restoreGState()
    }
}
...
public class RangeSlider: UIControl {
    ...
    @IBInspectable public var trackHighlightTintColor: UIColor = PersonalThemeNormalColor {
        didSet {
            trackLayer.setNeedsDisplay()
        }
    }
    ...
    func positionForValue(_ value: Double) -> Double {
        var result =  (Double(bounds.width - thumbWidth) * (value - minimumValue) /
    ...
    override public func beginTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        ...
        printDebugLog(message: "左侧滑轮")
        printDebugLog(message: lowerThumbLayer.frame)
        
        printDebugLog(message: "右侧滑轮")
        printDebugLog(message: upperThumbLayer.frame)
        printDebugLog(message: "点击点")
        printDebugLog(message: previouslocation)
        ...
        return isContain //lowerThumbLayer.highlighted || upperThumbLayer.highlighted
    }
