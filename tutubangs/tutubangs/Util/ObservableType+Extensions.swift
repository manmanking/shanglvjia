...
//    let test1Json = JSONDecoder.decode(LoginResponse.self,from:response)
    ...
    let dicResponse = try response.mapJSON()
...
//    guard dicResponse["code"].intValue == 100 else {
//        if(dicResponse["code"].intValue == 1024){
//            throw HttpError.companyNotBindUser(code: 1024, user: CompanyBindPersonForm(dicResponse["content"]))
//        }
//        throw HttpError.serverException(code: json["code"].intValue, message: json["message"].stringValue)
//        }
        return dicResponse as! Dictionary<String, Any>
    }
